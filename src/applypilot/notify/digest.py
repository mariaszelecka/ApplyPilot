...
# ── Outcome report (sent after an apply run) ─────────────────────────────
...
def _attach(msg, path, label: str) -> bool:
...
_MANUAL_REASONS = {
    "login_issue": "Site requires a login",
    "sso_required": "Site requires SSO sign-in",
    "captcha": "Blocked by CAPTCHA",
    "page_error": "Page could not be reached",
    "expired": "Posting expired",
    "not_eligible_location": "Outside the accepted locations",
}


def send_apply_outcome_report(applied: list[dict], manual: list[dict]) -> dict:
    """Report what an apply run actually achieved.

    `applied` went through; `manual` could not be submitted automatically
    (login wall, CAPTCHA, SSO) and needs finishing by hand. The tailored CV
    and cover letter for each manual job ride along as PDF attachments, so
    the mail is self-sufficient -- the application can be finished from a
    phone without going back to the machine to find the files.
    """
    load_env()
    profile = load_profile()
    gmail_address = os.environ.get("GMAIL_ADDRESS")
    ...
    msg["To"] = recipient
    msg["Subject"] = (
        f"ApplyPilot results: {len(applied)} submitted, {len(manual)} need you "
        f"({datetime.now().strftime('%Y-%m-%d')})"
    )

    def card(j, tone: str, note: str) -> str:
        colour = _C_ACCENT if tone == "ok" else "#B4541F"
        chip_bg = _C_PILL_BG if tone == "ok" else "#FBEEE4"
        return (
            f'<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"'
            f' style="background-color:{_C_CARD};border-radius:16px;margin:0 0 14px 0;">'
            f'<tr><td style="padding:20px 24px;font-family:Segoe UI,Helvetica,Arial,sans-serif;">'
            f'<div style="font-size:18px;font-weight:600;color:{_C_INK};line-height:1.3;">'
            f'{escape(str(j.get("title") or "Untitled"))}</div>'
            f'<div style="font-size:14px;color:{_C_ACCENT};padding-top:3px;">'
            f'{escape(str(j.get("company") or j.get("site") or "Unknown"))}</div>'
            f'<div style="padding-top:10px;">'
            f'<span style="display:inline-block;background-color:{chip_bg};color:{colour};'
            f'font-size:12px;font-weight:600;padding:4px 11px;border-radius:20px;">{escape(note)}</span>'
            f'<span style="font-size:13px;color:{_C_MUTED};padding-left:10px;">'
            f'{escape(str(j.get("location") or ""))}</span></div>'
            f'<div style="padding-top:14px;">'
            f'<a href="{escape(str(_job_link(j)), quote=True)}"'
            f' style="font-size:14px;color:{_C_ACCENT};text-decoration:none;font-weight:600;">'
            f'Open posting &rsaquo;</a></div>'
            f'</td></tr></table>'
        )
    ...
    H = [
        f'<div style="margin:0;padding:0;background-color:{_C_GROUND};">'
        f'<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"'
        f' style="background-color:{_C_GROUND};padding:28px 12px;"><tr><td align="center">'
        f'<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"'
        f' style="max-width:640px;width:100%;">'
        f'<tr><td style="padding:8px 12px 22px 12px;">'
        f'<div style="font-family:Segoe UI,Helvetica,Arial,sans-serif;font-size:34px;font-weight:300;'
        f'letter-spacing:-0.5px;color:{_C_ACCENT};line-height:1.15;">Application results</div>'
        f'<div style="font-family:Segoe UI,Helvetica,Arial,sans-serif;font-size:15px;color:{_C_MUTED};'
        f'padding-top:8px;line-height:1.55;">'
        f'<span style="color:{_C_INK};font-weight:600;">{len(applied)}</span> submitted automatically'
        f' &middot; <span style="color:{_C_INK};font-weight:600;">{len(manual)}</span> need you to finish'
        f'</div></td></tr>'
    ]

    if applied:
        H.append(
            f'<tr><td style="padding:0 0 6px 0;font-family:Segoe UI,Helvetica,Arial,sans-serif;'
            f'font-size:12px;font-weight:600;letter-spacing:.8px;text-transform:uppercase;'
            f'color:{_C_MUTED};">Submitted</td></tr><tr><td>'
        )
        for j in applied:
            H.append(card(j, "ok", "Applied"))
        H.append("</td></tr>")

    if manual:
        H.append(
            f'<tr><td style="padding:10px 0 6px 0;font-family:Segoe UI,Helvetica,Arial,sans-serif;'
            f'font-size:12px;font-weight:600;letter-spacing:.8px;text-transform:uppercase;'
            f'color:{_C_MUTED};">Needs you &mdash; CV and letter attached</td></tr><tr><td>'
        )
        for j in manual:
            reason = _MANUAL_REASONS.get(
                str(j.get("apply_error") or ""),
                str(j.get("apply_error") or "Could not submit"),
            )
            H.append(card(j, "manual", reason))
        H.append("</td></tr>")
    ...
    lines = [f"{len(applied)} submitted automatically, {len(manual)} need you.", ""]
    if applied:
        ...
        lines.append("")
    if manual:
        lines.append("NEEDS YOU (CV + cover letter attached)")
        for j in manual:
            reason = _MANUAL_REASONS.get(
                str(j.get("apply_error") or ""),
                str(j.get("apply_error") or "could not submit"),
            )
            lines.append(f"  - {j.get('title')} @ {j.get('company') or '?'} [{reason}]")
            lines.append(f"    {_job_link(j)}")
        lines.append("")

    alt = MIMEMultipart("alternative")
    alt.attach(MIMEText("\n".join(lines), "plain"))
    alt.attach(MIMEText("".join(H), "html"))
    msg.attach(alt)
    ...
    for j in manual:
        ...
        if cv and _attach(msg, cv, f"CV_{tag}"):
            attached += 1
        if cl and _attach(msg, cl, f"Letter_{tag}"):
            attached += 1
    ...
    try:
        ...
        log.error("Apply outcome report: send failed: %s", e)
        return {"sent": 0, "error": str(e)}

    log.info("Apply outcome report sent to %s: %d applied, %d manual, %d attachments",
             recipient, len(applied), len(manual), attached)
    return {"sent": 1, "applied": len(applied), "manual": len(manual),
            "attachments": attached, "error": None}
